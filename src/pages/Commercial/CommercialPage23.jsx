...
export default function CommercialPage23() {
  const navigate = useNavigate();
  const { commercialPages } = useProposalStore();
  const pageData = commercialPages?.page23;

  const sections = pageData?.sections || pageData?.items || [];
  const isPage21 = 23 === 21;
    ...
    <div className="w-full min-h-screen bg-[#eef2f7] flex justify-center py-8">
      <div className="w-[1150px] bg-white border border-[#d9d9d9] shadow-lg px-[40px] pt-[25px] pb-[60px]">
        ...
        {pageData ? (
          ...
          <div className="mt-6 text-[13px] text-gray-400 italic">No data found for page 23. Check commercialPages19to24Data.js.</div>
        )}
        ...
        <div className="mt-8 flex justify-between items-end text-[10px] italic text-[#8d8d8d]">
          ...
          <div className="text-right">
            ...
            <p className="text-[#003f77] font-semibold not-italic mt-1">Page 23 of 24</p>
          </div>
        </div>

        {/* <div className="flex justify-between mt-8">
          <button onClick={() => navigate("/Commercial/CommercialPage22")} className="px-5 py-2 rounded-full border border-[#0d4c7f] text-[#0d4c7f] text-[12px] font-semibold bg-white hover:bg-gray-50">← Previous Page</button>
          <button onClick={() => navigate("/Commercial/CommercialPage24")} className="px-5 py-2 rounded-full bg-[#0d4c7f] text-white text-[12px] font-semibold hover:bg-[#0b3f6a]">Next →</button>
        </div> */}
