...
export default function Page10() {
  ...
  const storeData = technicalPages.page10 || {};
  const initialData = technicalPagesData[systemType]?.page10?.sections || [];
  ...
  useEffect(() => {
    if (technicalPages.page10?.sections && technicalPages.page10.sections.length > 0) {
      setSections(technicalPages.page10.sections);
    } else {
      setSections(initialData);
      // Save to store if using fallback data
      if (initialData.length > 0 && !technicalPages.page10?.sections) {
        updateTechnicalPage("page10", { sections: initialData });
      }
    }
  }, [systemType, technicalPages.page10, initialData]);

  // Save to store whenever sections change
  const updateSections = (newSections) => {
    setSections(newSections);
    updateTechnicalPage("page10", { sections: newSections });
  };
            ...
            <div className="flex justify-center gap-10 mt-10">
              {section.images.map((image, imageIndex) => (
                <div key={imageIndex} className="flex flex-col items-center">
                  ...
                  />
                  {image.caption && (
                    <div className="mt-3 text-center italic text-[16px]">
                      {image.caption}
                    </div>
                  )}
                </div>
              ...
              <div key={imageIndex} className="flex flex-col items-center">
                ...
                <div className="relative mt-3">
                  <textarea
                    ...
                    rows={1}
                    className="resize-none overflow-hidden border-none outline-none bg-transparent text-center italic text-[16px]"
                    placeholder="Enter caption..."
                  />
                  <Pencil size={14} className="absolute right-[-18px] top-1 text-slate-400" />
                </div>
              </div>
            ))}
          </div>
