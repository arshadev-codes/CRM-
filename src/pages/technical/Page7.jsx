...
export default function Page7() {
  ...
  const storeData = technicalPages.page7 || {};
  const initialData = technicalPagesData[systemType]?.page7?.sections || [];
  ...
  useEffect(() => {
    if (technicalPages.page7?.sections && technicalPages.page7.sections.length > 0) {
      setSections(technicalPages.page7.sections);
    } else {
      setSections(initialData);
      // Save to store if using fallback data
      if (initialData.length > 0 && !technicalPages.page7?.sections) {
        updateTechnicalPage("page7", { sections: initialData });
      }
    }
  }, [systemType, technicalPages.page7, initialData]);

  // Save to store whenever sections change
  const updateSections = (newSections) => {
    setSections(newSections);
    updateTechnicalPage("page7", { sections: newSections });
  };
          ...
          {section.images?.[0]?.src && (
            <div className="mt-10 flex justify-center">
              <img 
                ...
                alt="" 
                className="w-[760px] object-cover border border-slate-300" 
              />
            </div>
          )}
        </div>
