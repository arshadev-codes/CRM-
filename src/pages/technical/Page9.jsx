...
export default function Page9() {
  ...
  const storeData = technicalPages.page9 || {};
  const initialData = technicalPagesData[systemType]?.page9?.sections || [];
  ...
  useEffect(() => {
    if (technicalPages.page9?.sections && technicalPages.page9.sections.length > 0) {
      setSections(technicalPages.page9.sections);
    } else {
      setSections(initialData);
      // Save to store if using fallback data
      if (initialData.length > 0 && !technicalPages.page9?.sections) {
        updateTechnicalPage("page9", { sections: initialData });
      }
    }
  }, [systemType, technicalPages.page9, initialData]);

  // Save to store whenever sections change
  const updateSections = (newSections) => {
    setSections(newSections);
    updateTechnicalPage("page9", { sections: newSections });
  };
  ...
  reader.onload = (event) => {
    ...
    updated[sectionIndex].images[imageIndex].src =
      event.target.result;

    setSections(updated);

    updateTechnicalPage("page9", {
      sections: updated,
    });
  };
            ...
            <div className="grid grid-cols-2 gap-12 mt-10">
              {section.images.map((image, imageIndex) => (
                <img 
                  ...
                  src={image.src} 
                  alt="" 
                  className="w-full h-[520px] object-cover border border-slate-300" 
                />
              ))}
            </div>
          )}
      ...
      <div className={`w-[1150px] bg-white border border-slate-300 shadow-xl rounded-xl px-[70px] pt-[55px] pb-[120px] relative ${isPrintMode ? 'shadow-none' : ''}`}>
        ...
        {!isPrintMode && (
          <div className="flex justify-between items-center mt-12">
            <button
              onClick={() => navigate("/technical/page8")}
