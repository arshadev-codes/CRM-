...
export default function Page8() {
  ...
  const storeData = technicalPages.page8 || {};
  const initialData = technicalPagesData[systemType]?.page8?.sections || [];
  ...
  useEffect(() => {
    if (technicalPages.page8?.sections && technicalPages.page8.sections.length > 0) {
      setSections(technicalPages.page8.sections);
    } else {
      setSections(initialData);
      // Save to store if using fallback data
      if (initialData.length > 0 && !technicalPages.page8?.sections) {
        updateTechnicalPage("page8", { sections: initialData });
      }
    }
  }, [systemType, technicalPages.page8, initialData]);

  // Save to store whenever sections change
  const updateSections = (newSections) => {
    setSections(newSections);
    updateTechnicalPage("page8", { sections: newSections });
  };
          ...
          {section.images?.[0]?.src && (
            <div className="mt-10 flex justify-center">
              <img 
                ...
                alt="" 
                className="w-[620px] object-cover border border-slate-300" 
              />
            </div>
          )}
        </div>
