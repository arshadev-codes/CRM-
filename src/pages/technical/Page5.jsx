...
export default function Page5() {
  ...
  const storeData = technicalPages.page5 || {};
  const initialData = technicalPagesData[systemType]?.page5?.sections || [];
  ...
  useEffect(() => {
    if (technicalPages.page5?.sections && technicalPages.page5.sections.length > 0) {
      setSections(technicalPages.page5.sections);
    } else {
      setSections(initialData);
      // Save to store if using fallback data
      if (initialData.length > 0 && !technicalPages.page5?.sections) {
        updateTechnicalPage("page5", { sections: initialData });
      }
    }
  }, [systemType, technicalPages.page5, initialData]);

  // Save to store whenever sections change
  const updateSections = (newSections) => {
    setSections(newSections);
    updateTechnicalPage("page5", { sections: newSections });
  };
            ...
            {section.images.slice(1).map((image, imageIndex) => (
              <div key={imageIndex} className="relative">
                <img 
                  ...
                  className="w-full border border-slate-300" 
                />
                <label className="absolute top-3 right-3 bg-white border border-slate-200 p-2 rounded-full shadow-md cursor-pointer">
                  <ImagePlus size={16} />
                  <input
                    type="file"
                    hidden
                    onChange={(e) => updateImage(sectionIndex, imageIndex + 1, e.target.files[0])}
                  />
                </label>
              </div>
            ))}
      ...
      <div className={`w-[1150px] bg-white border border-slate-300 shadow-xl rounded-xl px-[70px] pt-[55px] pb-[120px] relative ${isPrintMode ? 'shadow-none' : ''}`}>
        ...
        {!isPrintMode && (
          <div className="flex justify-between items-center mt-12">
            ...
            <button
              onClick={() => navigate("/technical/page6")}
