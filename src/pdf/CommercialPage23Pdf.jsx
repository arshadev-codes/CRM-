...
/* Handles both bare subsections (11.4, 11.5) and titled sections (12, 13...) */
function TcEntry({ section }) {
  const isBareSubsection = !section.title && section.body;
...
export default function CommercialPage23Pdf() {
      ...
      >
        <div style={{ flex: 1, overflow: "hidden" }}>
          {(pageData.sections || []).map((section, idx) => (
            <TcEntry key={idx} section={section} />
          ))}
        </div>
      </div>

      <PdfFooter pageNumber={23} />
    </div>
  );
}